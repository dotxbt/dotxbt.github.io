...
const SectionCard = async ({ headerTitle, data, colorBg, colorFg }) => {
  ...
  cardChild.classList = `flex grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 2xl:grid-cols-8  py-4 gap-4 px-4 w-full`;
  ...
  await fetch(data)
    ...
    .then((data) => {
      ...
      <a ${
        e.url
      } class="flex bg-white rounded-xl flex-col cursor-pointer justify-start items-center min-w-[100px] hover:animate-bounce p-4 rounded-2xl">
        ${
          e.logo
            ? `<img class=" h-[100px] w-full px-4 flex-1 object-contain" src="${e.logo}"/>`
            : `<i class="${e.icon} text-[80px] text-center w-full flex flex-col items-center justify-center flex-1 h-[80px] ${e.color}"></i>`
        }  
        <p class="text-xl ${colorFg} mt-4 text-center text-slate-700">${e.title}</p>
      </a> 
      `
          )
        : "Loading....";
