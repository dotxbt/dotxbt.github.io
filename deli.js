...
const LinkCard = async () => {
  ...
  cardChild.classList = `flex flex-col space-y-6 w-full p-4 max-w-sm mb-8`;
  ...
  await fetch("./deli.json")
    ...
    .then((data) => {
      cardChild.innerHTML = data
        ? data.map(
            (e) => `
      <a  href="${e.url}" target="_blank" active:opacity-50 rel="noopener noreferrer" class="flex flex-row cursor-pointer w-full shadow-2xl items-center bg-white/60 rounded-xl p-3">
        <img class=" h-[46px] w-[46px] object-contain" src="${e.logo}" width={64} height={64}/>
        <p class="text-2xl text-slate-800 font-bold flex-1 text-center">${e.title}</p>
      </a> 
      `
          )
        : "Loading....";
